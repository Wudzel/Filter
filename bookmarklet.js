...
bar.appendChild(dots);bar.appendChild(ul);
...
style.textContent='@keyframes __mbspin{to{transform:rotate(360deg)}}';
...
rs.style.cssText='position:absolute;bottom:0;right:0;width:28px;height:28px;cursor:se-resize;touch-action:none;z-index:10;display:flex;align-items:flex-end;justify-content:flex-end;padding:5px;';
rs.innerHTML='<svg width="13" height="13" viewBox="0 0 13 13" fill="white" opacity="0.3"><path d="M13 9L9 13H13V9ZM13 4.5L4.5 13H7L13 7V4.5ZM13 0L0 13H2.5L13 2.5V0Z"/></svg>';
ov.appendChild(bar);ov.appendChild(ct);ov.appendChild(rs);
document.body.appendChild(ov);

function loadUrl(url){
  sp.style.display='block';fr.style.display='none';ul.textContent=url;
  fetch(PROXY+encodeURIComponent(url))
    .then(function(r){return r.json();})
    .then(function(d){
      var html=d.contents;
      var base=new URL(url);
      html=html.replace(/((?:src|href|action|data-src)\s*=\s*["'])(?!https?:\/\/|\/\/|#|data:|javascript:|mailto:)([^"']*?)(["'])/gi,function(m,a,v,q){
        try{return a+new URL(v,base).href+q;}catch(e){return m;}
      });
      html=html.replace(/<\/head>/i,'<script>document.addEventListener("click",function(e){var a=e.target.closest("a");if(a&&a.href&&!a.href.startsWith("javascript:")){e.preventDefault();window.parent.postMessage({__mb:true,url:a.href},"*");}});<\/script></head>');
      fr.srcdoc=html;
      fr.onload=function(){sp.style.display='none';fr.style.display='block';};
    })
    .catch(function(){sp.style.display='none';ct.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:12px;text-align:center;padding:20px;">Could not load page.<br>Check your connection.</div>';});
}
